        ...
        async with semaphore: 
            ...
            if feed_id:
                ...
                if new_articles_added:
                   await Database.feeds.update_one({'_id': feed_id}, {'$set': {'last_updated': datetime.now(UTC)}})

    @staticmethod
    async def get_or_create_feed(url: str, feed_data: dict, feed_language: str):
        existing = await Database.feeds.find_one({'feed': url})
        ...
        if existing:
            ...
            if existing.get('language') != feed_language:
                update_data['language'] = feed_language

            await Database.feeds.update_one({'_id': existing['_id']}, {'$set': update_data})
        ...
        logging.info(f"New feed inserted: {new_feed.get('title')} ({url})")
