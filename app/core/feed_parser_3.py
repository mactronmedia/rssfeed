...
class VideosModule:
    ...
    def channel_metadata(feed_url, feed_type):
        ...
        VideosModule.collection_feeds.insert_one(feed_data)
    ...
    def video_items(feed_url):
        for entry in feed.entries[:500]:  # Limit to 5 entries
            ...
            video_id = entry.get('yt_videoid', None)  # yt:videoId is typically parsed to 'yt_videoid'
            channel_id = entry.get('yt_channelid', None)  # yt:channelId is typically parsed to 'yt_channelid'


            print(f"Video ID: (yt:videoId): {video_id}")
            print(f"Channel ID: (yt:channelId): {channel_id}")
            ...
            VideosModule.collection_videos.insert_one(video_data)
