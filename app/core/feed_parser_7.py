...
class Database:
    client = AsyncIOMotorClient('mongodb://localhost:27017/')  # Async MongoDB client
...
async def main():
    ...
    feed = feedparser.parse(feed_url)  # Note: This is still synchronous
    feed_type = await FeedProcessor.get_feed_type(feed, feed_url)
