...
Parameters:
- parse_image: Optional flag (1 or 0) to control whether to parse the channel avatar image
  - 1: Parse the image URL (default)
  - 0: Skip image parsing
"""
...
class YouTubeChannel:
    @staticmethod
    async def get_channel_info(entry: str, parse_image: int = 1) -> Dict[str, str]:
        entry = YouTubeChannel.normalize_entry(entry)

        if YouTubeChannel.is_rss_feed(entry):
            return await YouTubeChannel.process_rss_feed(entry, parse_image)
        ...
        async with aiohttp.ClientSession() as session:
            ...
            json_data = YouTubeChannel.extract_info_from_json(html, parse_image)
            if json_data:
                return json_data
            ...
            avatar_url = feed_data.feed.get('icon', None) if parse_image else None
    ...
    @staticmethod
    def extract_info_from_json(html: str, parse_image: int = 1) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            # Extract avatar URL with preferred size if enabled
            avatar_url = YouTubeChannel.extract_avatar_url(json_data, preferred_size=240) if parse_image else None
    ...
    def extract_avatar_url(json_data: dict, preferred_size: int = 240) -> Optional[str]:
        ...
        def process_avatar_url(url: str) -> str:
            ...
            if 'yt3.googleusercontent.com' in url:
                # Example pattern: https://yt3.googleusercontent.com/...=s176-c-k-c0x00ffffff-no-rj
                # We can modify the 's176' part to get different sizes
    ...
    @staticmethod
    async def process_rss_feed(entry: str, parse_image: int = 1) -> Dict[str, str]:
        parsed = YouTubeFeedParser.parse_feed(entry, parse_image)
        return parsed if parsed else {"error": "Failed to parse RSS feed."}
...
if __name__ == "__main__":
    ...
    async def main():
        entry = 'DoctorMike'
        logger.info(f"\nTesting with: {entry}")
        result = await YouTubeChannel.get_channel_info(entry, parse_image=1)
        ...
        if 'error' in result:
            ...
            logger.info(f"Avatar URL    : {result.get('avatar_url', 'Not found')}")
