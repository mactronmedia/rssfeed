...
def retry_request(retries: int = 3, timeout: int = 5):
    """Decorator to handle retries and timeouts for requests with configurable retry count."""
    def decorator(func):
        @wraps(func)
...
class YouTubeChannel:
    ...
    @staticmethod
    @retry_request(retries=5, timeout=3)  # You can adjust retries and timeout here
    def fetch_page(session: requests.Session, url: str) -> requests.Response:
        ...
        return session.get(url)  # Timeout is handled by the decorator
