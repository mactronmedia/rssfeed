...
class RSSParser:
    ...
    async def fetch_feed(url: str, session: aiohttp.ClientSession) -> Dict[str, Any]:
        try:
            async with session.get(url) as response:
                ...
                # Detect language of the feed's description or title
    ...
    async def get_or_create_feed(url: str, feed_data: dict, feed_language):
        ...
        result = await Database.feeds.insert_one(doc)
    ...
    async def process_articles(entries: List[dict], feed_id, session: aiohttp.ClientSession, feed_language: str):
        ...
        for entry in entries:
            ...
            # Detect the language of the article description
            description = entry.get('description', '')
            article_language = RSSParser.detect_language(description)
            logging.info(f"Article {link} detected language: {article_language}")
            ...
            articles.append({
                ...
                'thumbnail': thumbnail,
                'language': article_language,  # Store the detected language
                'last_updated': datetime.now(UTC)
            })
