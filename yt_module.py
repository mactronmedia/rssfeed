...
class YouTubeChannel:
    ...
    async def get_channel_info(entry: str, session: aiohttp.ClientSession) -> dict:
        ...
        if not YouTubeChannel.is_valid_youtube_url(entry):
            return {"error": f"Invalid YouTube URL: {entry}"}
    ...
    def is_valid_youtube_url(url: str) -> bool:
        """
        Validate that the URL belongs to YouTube.com.
        """
        try:
            ...
            return netloc in valid_hosts or netloc.endswith(".youtube.com") or netloc == "youtu.be"
    ...
    def normalize_entry(entry: str) -> str:
        ...
        if entry.startswith('@'):
            return f"{YOUTUBE_BASE_URL}/{entry}"
    ...
    async def get_channel_avatar(channel_url_or_id: str, session: ClientSession, preferred_size: int = 240) -> Optional[str]:
        ...
        if not channel_url_or_id.startswith('http'):
            if channel_url_or_id.startswith('@'):
                channel_url = f"{YOUTUBE_BASE_URL}/{channel_url_or_id}"
            elif channel_url_or_id.startswith('UC'):
                channel_url = f"{YOUTUBE_BASE_URL}/channel/{channel_url_or_id}"
            else:
                channel_url = f"{YOUTUBE_BASE_URL}/{channel_url_or_id}"
        else:
...
class YouTubeParser:
    ...
    async def create_new_feed(feed_url: str, feed, session: ClientSession):
        ...
        channel_url = (
            ...
            f"{YOUTUBE_BASE_URL}/channel/{channel_id}" if channel_id else None
        )
    ...
    async def video_items(feed, session: ClientSession, feed_object_id):
        ...
        for entry in feed.entries:
            tasks.append(YouTubeParser.process_video_entry(entry, feed_object_id))
    ...
    @staticmethod
    async def process_video_entry(entry, feed_object_id):
