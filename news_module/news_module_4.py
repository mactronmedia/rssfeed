...
class Database:
    ...
    @staticmethod
    async def update_article_thumbnail(article_id: str, thumbnail_url: str) -> None:
    ...
    async def process_article_thumbnails(articles: List[Dict], session: ClientSession):
        ...
        for article in articles:
            ...
            if await RSSParser.validate_image_url(article['thumbnail'], session):
                await Database.update_article_thumbnail(article['article_id'], article['thumbnail'])
            else:
                # If primary thumbnail is invalid, try to find a valid one from potential sources
                potential_thumbnails = RSSParser.extract_potential_thumbnails(article)
                for thumbnail_url in potential_thumbnails:
                    if await RSSParser.validate_image_url(thumbnail_url, session):
                        await Database.update_article_thumbnail(article['article_id'], thumbnail_url)
                        break

    @staticmethod
    def extract_potential_thumbnails(article: Dict) -> List[str]:
        """Extract potential thumbnail URLs from article data."""
        thumbnails = []
        
        # Check media_content for images
        for media in article.get('media_content', []):
            if media.get('medium') == 'image' or media.get('type', '').startswith('image/'):
                thumbnails.append(media.get('url', ''))
        
        # Check enclosures for images
        for enc in article.get('enclosures', []):
            if enc.get('type', '').startswith('image/'):
                thumbnails.append(enc.get('url', ''))
        
        # Check content for embedded images
        if article.get('content'):
            thumbnails.extend(RSSParser.extract_images_from_html(article['content']))
        
        # Deduplicate
        return list(set([url for url in thumbnails if url]))
