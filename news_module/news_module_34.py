...
class RSSParser:
    ...
    async def get_or_create_feed(url: str, feed_data: dict, feed_language: str):
        ...
        new_feed = {
            ...
            'last_updated': datetime.now(UTC), ### here i woul add if new entires/news has been added 
        }
    ...
    async def process_articles(entries: List[dict], feed_id, session: aiohttp.ClientSession) -> bool:
        ...
        existing = await Database.articles.find({"link": {"$in": all_links}}).to_list(length=None)
        ...
        return bool(inserted_ids)  # True if any article was inserted
