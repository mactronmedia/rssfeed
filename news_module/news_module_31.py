...
class RSSParser:
    ...
    async def get_or_create_feed(url: str, feed_data: dict, feed_language: str):
        ...
        if existing:
            ...
            # Convert the existing 'last_updated' to a timezone-aware datetime in UTC
            last_updated = existing.get('last_updated')
            if last_updated and last_updated.tzinfo is None:
                last_updated = last_updated.replace(tzinfo=timezone.utc)  # Make it aware if it's naive

            # Check if new entries have been added by comparing the last updated time
            if last_updated and last_updated < datetime.now(timezone.utc):  # Use UTC aware datetime
                await Database.feeds.update_one(
                    {'_id': existing['_id']},
                    {'$set': {'last_updated': datetime.now(timezone.utc)}}  # Update last_updated if new entries are found
                )
        ...
        new_feed = {
            ...
            'last_updated': datetime.now(timezone.utc),  # Use UTC aware datetime for new feeds
        }
    ...
    async def process_articles(entries: List[dict], feed_id, session: aiohttp.ClientSession):
        ...
        for entry in entries:
            try:
                ...
                if article:
                    ...
                    new_entries_added = True  # Mark that new entries have been added
            except Exception as e:
        ...
        # If new articles are added, update the feed's last_updated timestamp
